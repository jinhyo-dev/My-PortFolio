...
import styles from '../../../styles/mobileComponents.module.scss'
...
const Contact = () => {
  ...
  return (
    <div>
      <h1 className={cs('AboutMe-title')}>Contact</h1>
      ...
      <div className={cs('contactContainer')}>
        <FaInstagram className={cs('instagram', 'icon')} size={'55'} onClick={() => pushToPage('instagram')}
                     onMouseEnter={() => setInstagramIsHover(true)} onMouseLeave={() => setInstagramIsHover(false)}/>
        <GrMail className={cs('mail', 'icon')} size={'65'} onClick={() => pushToPage('mail')}
                onMouseEnter={() => setmailIsHover(true)} onMouseLeave={() => setmailIsHover(false)}/>
        <BsGithub className={cs('github', 'icon')} size={'50'} onClick={() => pushToPage('github')}
                  onMouseEnter={() => setgithubIsHover(true)} onMouseLeave={() => setgithubIsHover(false)}/>
        <HiOutlineCode className={cs('code', 'icon')} size={'65'} onClick={() => pushToPage('solvedac')}
                       onMouseEnter={() => setCodeIsHover(true)} onMouseLeave={() => setCodeIsHover(false)}/>
        <SiDiscord className={cs('discord', 'icon')} size={'60'}
